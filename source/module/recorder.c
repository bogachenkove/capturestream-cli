...
recorder_context *recorder_create (const char *streamer, const char *platform, const char *base_dir, const void *config)
{
 ...
 snprintf (context->work_directory, sizeof (context->work_directory), "%s/%s_%s", base_dir, platform, streamer);

 if (mkdir (context->work_directory, 0755) != 0 && errno != EEXIST)
 {
		log_write (LOG_ERROR, "recorderCreate: mkdir failed for %s: %s", context->work_directory, strerror (errno));
		free (context);
		return NULL;
 }

 snprintf (context->log_file_path, sizeof (context->log_file_path), "%s/%s_%s.log", base_dir, platform, streamer);
 ...
 log_write (LOG_INFO, "recorderCreate: created for %s/%s with quality %s", platform, streamer, context->stream_quality);
 return context;
}
...
void recorder_update_configuration (recorder_context *context, const void *new_config)
{
 ...
 log_write (LOG_INFO, "recorderUpdateConfiguration: updated for %s, quality=%s", context->streamer_name, context->stream_quality);
}
...
void recorder_get_status_string (recorder_context *context, char *buffer, size_t buffer_size)
{
 ...
 snprintf (buffer, buffer_size, "%s/%s: %s (failures: %d)", context->platform_name, context->streamer_name, state_name, context->consecutive_failures);
...
static int is_stream_live (const char *streamer, const char *platform)
{
 ...
 char command[512];
 snprintf (command, sizeof (command), "streamlink --json https://www.twitch.tv/%s 2>/dev/null", escaped_streamer);
...
static pid_t launch_streamlink_and_ffmpeg (recorder_context *context, const char *stream_url)
{
 ...
 if (process_id == 0)
 {
		...
		char command[4096];
		int written = snprintf (command, sizeof (command), "streamlink --stdout --ipv4 --stream-timeout %d --retry-streams %d --retry-open %d %s %s 2>>%s | "
																										...
																										context->stream_timeout_seconds, context->retry_streams_seconds, context->retry_open_attempts, escaped_url, quality_chain, escaped_log, context->segment_duration_seconds, context->work_directory, context->platform_name, context->streamer_name, escaped_log);
		...
		log_write (LOG_INFO, "launchStreamlinkAndFfmpeg: using quality chain '%s' for %s", quality_chain, context->streamer_name);
...
static void *recorder_thread_function (void *argument)
{
 ...
 while (!atomic_load (&context->stop_requested))
 {
		if (!is_stream_live (context->streamer_name, context->platform_name))
		{
			...
			log_write (LOG_DEBUG, "recorderThread: %s offline, waiting %d s", context->streamer_name, context->retry_streams_seconds);
		...
		log_write (LOG_INFO, "recorderThread: %s is live, starting recording with quality '%s'", context->streamer_name, context->stream_quality);
		...
		if (duration < 10)
		{
			log_write (LOG_INFO, "recorderThread: %s recording ended quickly (%ld s), waiting", context->streamer_name, duration);
		...
		{
			log_write (LOG_INFO, "recorderThread: %s recording finished after %ld s", context->streamer_name, duration);
...
static void handle_child_exit (recorder_context *context, pid_t process_id, int status)
{
 if (WIFEXITED (status))
 {
		...
		log_write (LOG_DEBUG, "recorderThread: child %d exited with code %d for %s", process_id, exit_code, context->streamer_name);
 ...
 else if (WIFSIGNALED (status))
 {
		...
		log_write (LOG_WARN, "recorderThread: child %d killed by signal %d for %s", process_id, signal_number, context->streamer_name);
