...
static void build_mp4_output_path (streamer_assembly_state *assembly_state, const char *first_ts_filename, char *output_path, size_t output_size);
...
static void update_states (assembler_context *context)
{
 ...
 if (total_recorders > 0)
 {
		...
		for (int streamer_position = 0; streamer_position < total_recorders; streamer_position++)
		{
			...
			if (streamer_manager_get_streamer_info (streamer_manager_handle, streamer_position, streamer_name, sizeof (streamer_name), platform_name, sizeof (platform_name), work_directory, sizeof (work_directory)) == 0)
			{
...
static void build_mp4_output_path (streamer_assembly_state *assembly_state, const char *first_ts_filename, char *output_path, size_t output_size)
{
 ...
 snprintf (first_file_path, sizeof (first_file_path), "%s/%s", assembly_state->work_directory, first_ts_filename);

 if (stat (first_file_path, &file_status) == 0)
 {
		file_time = file_status.st_mtime;
 }

 struct tm time_components;
 localtime_r (&file_time, &time_components);
 char time_str[32];
 strftime (time_str, sizeof (time_str), "%Y%m%d_%H%M%S", &time_components);

 snprintf (output_path, output_size, "%s/%s_%s_%s.mp4", assembly_state->work_directory, assembly_state->platform_name, assembly_state->streamer_name, time_str);
}
...
static void process_streamer (assembler_context *context, streamer_assembly_state *assembly_state, int finalize)
{
 ...
 time_t newest_modification_time = 0;
 for (int file_index = 0; file_index < assembly_state->pending_count; file_index++)
 {
		struct stat file_status;
		char complete_path[512];
		snprintf (complete_path, sizeof (complete_path), "%s/%s", assembly_state->work_directory, assembly_state->pending_files[file_index]);
		if (stat (complete_path, &file_status) == 0)
		{
			if (file_status.st_mtime > newest_modification_time)
			{
				newest_modification_time = file_status.st_mtime;
			}
		}
 }

 time_t current_time = time (NULL);
 if (newest_modification_time > 0 && (current_time - newest_modification_time) >= 300)
 {
		log_write (LOG_INFO, "processStreamer: no new segments for %s in 5 mins, forcing finalize", assembly_state->streamer_name);
		finalize = 1;
 }
 ...
 for (int file_position = 0; file_position < total_to_process; file_position++)
 {
		struct stat file_status;
		char complete_path[512];
		snprintf (complete_path, sizeof (complete_path), "%s/%s", assembly_state->work_directory, processing_list[file_position]);
		if (stat (complete_path, &file_status) != 0)
		{
			continue;
		}
		...
		if (accumulated_size + current_file_size > maximum_bytes && (file_position - start_position) > 0)
		{
			...
			build_mp4_output_path (assembly_state, processing_list[start_position], output_path, sizeof (output_path));

			if (concat_files (assembly_state, &processing_list[start_position], count_to_concat, output_path) == 0)
			{
				if (!context->current_config.concat_keep_ts)
				{
					...
					{
						char file_path[512];
						snprintf (file_path, sizeof (file_path), "%s/%s", assembly_state->work_directory, processing_list[removal_position]);
						unlink (file_path);
					}
				}
				for (int removal_position = start_position; removal_position <= end_position; removal_position++)
 ...
 {
		char output_path[1024];
		build_mp4_output_path (assembly_state, assembly_state->pending_files[0], output_path, sizeof (output_path));

		if (concat_files (assembly_state, assembly_state->pending_files, assembly_state->pending_count, output_path) == 0)
		{
			if (!context->current_config.concat_keep_ts)
			{
				...
				{
					char file_path[512];
					snprintf (file_path, sizeof (file_path), "%s/%s", assembly_state->work_directory, assembly_state->pending_files[removal_position]);
					unlink (file_path);
				}
			}
			for (int removal_position = 0; removal_position < assembly_state->pending_count; removal_position++)
...
static void scan_directory (streamer_assembly_state *assembly_state)
{
 ...
 snprintf (name_prefix, sizeof (name_prefix), "%s_%s_", assembly_state->platform_name, assembly_state->streamer_name);
 ...
 for (int entry_position = 0; entry_position < entry_total; entry_position++)
 {
		if (directory_entries[entry_position]->d_type == DT_REG)
		{
			...
			if (strncmp (entry_name, name_prefix, strlen (name_prefix)) == 0)
			{
				...
				{
					assembly_state->pending_files = realloc (assembly_state->pending_files, (assembly_state->pending_count + 1) * sizeof (char *));
...
static int concat_files (streamer_assembly_state *assembly_state, char **file_list, int file_count, const char *output_path)
{
 ...
 snprintf (list_file_path, sizeof (list_file_path), "%s/ffmpeg_list_%d.txt", assembly_state->work_directory, getpid ());
 ...
 snprintf (shell_command, sizeof (shell_command), "ffmpeg -nostats -loglevel warning -f concat -safe 0 -i '%s' -c copy -movflags +faststart \"%s\" -y </dev/null 2>>%s/ffmpeg_assembler.log", list_file_path, output_path, assembly_state->work_directory);
