...
int config_watcher_start (configuration_manager *manager)
{
 ...
 {
		log_write (LOG_ERROR, "configWatcherStart: inotify_add_watch failed for %s: %s", manager->current_configuration.configuration_path, strerror (errno));
 ...
 if (pthread_create (&manager->reload_thread_identifier, NULL, inotify_thread_function, manager) != 0)
 {
 ...
 log_write (LOG_INFO, "configWatcherStart: watcher started for %s", manager->current_configuration.configuration_path);
 return 0;
}
