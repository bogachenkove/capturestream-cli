...
int command_handler_process (control_server *server, const char *command_text, char *response_text, size_t response_size)
{
 ...
 if (strcmp (command_token, "status") == 0)
 {
		...
		if (line_count > 0)
		{
			...
			{
				response_position += snprintf (response_text + response_position, response_size - response_position, "%s\n", status_lines[status_index]);
