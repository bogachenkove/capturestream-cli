...
streamer_manager *streamer_manager_create (const void *initial_config)
{
 ...
 if (strlen (configuration_snapshot->twitch_streamers) > 0)
 {
		...
		if (streamer_list_parse (configuration_snapshot->twitch_streamers, &names, &name_count) == 0)
		{
			...
			{
				recorder_context *context = recorder_create (names[streamer_index], "twitch", configuration_snapshot->base_directory, configuration_snapshot);
				if (context != NULL)
				{
...
int streamer_manager_reload_configuration (streamer_manager *manager, const void *new_config)
{
 ...
 for (int new_index = 0; new_index < new_count; new_index++)
 {
		...
		if (!presence)
		{
			recorder_context *context = recorder_create (new_names[new_index], "twitch", configuration_snapshot->base_directory, configuration_snapshot);
			if (context != NULL)
			{
				if (recorder_start (context) == 0)
				{
...
int streamer_manager_remove_streamer (streamer_manager *manager, const char *streamer, const char *platform, int finalize)
{
 if (manager == NULL || streamer == NULL || platform == NULL)
 {
		return -1;
...
int streamer_manager_get_streamer_info (streamer_manager *manager, int index, char *name, size_t name_size, char *platform, size_t platform_size, char *work_dir, size_t dir_size)
{
